	...
	public int findCenterSeq() {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = i + 1; j < n; j++) {
				 ...
				 * System.out.println(String.valueOf(tmp[0][tt])+"|"+String.
				 * valueOf(tmp[1][tt])+"|"+String.valueOf(tmp[2][tt])); }
				 **********/

			}
			for (int j = 0; j < i; j++) {
				Num[i][j] = Num[j][i];
				int[][] tmp = new int[3][Name[j][i][0].length];
				for(int k=0;k<Name[j][i][0].length;k++){
					
					tmp[0][k] = Name[j][i][1][k];
					tmp[1][k] = Name[j][i][0][k];
					tmp[2][k] = Name[j][i][2][k];
